...
import hou, os, re, toolutils, subprocess
import pyperclip
from PySide.QtCore import Qt

class HShots (QtGui.QWidget):
    ...
    def __init__(self):
        ...
        shotLay = QtGui.QHBoxLayout()
        btns = QtGui.QHBoxLayout()
        btns2 = QtGui.QHBoxLayout()
        self.coll = QtGui.QGroupBox('Settings')
        self.coll.setCheckable(True)
        self.coll.setChecked(True)
        vbox =  QtGui.QVBoxLayout()
        self.shots = QtGui.QTreeWidget()
        ...
        self.ln_name = QtGui.QLineEdit()
        self.ln_notes = QtGui.QLineEdit()
        self.ln_name.setPlaceholderText('Shot name')
        self.ln_notes.setPlaceholderText('Additional notes')
        self.ln_begin = QtGui.QLineEdit()
        self.ln_end = QtGui.QLineEdit()
        ...
        self.combo_cam = QtGui.QComboBox()
        self.combo_cam.setMaxVisibleItems(6)
        self.combo_cam.setMinimumContentsLength(10)
        self.combo_bundle = QtGui.QComboBox()
        self.combo_bundle.setMaxVisibleItems(6)
        self.combo_takes = QtGui.QComboBox()
        self.combo_takes.setMaxVisibleItems(6)
        self.ch = QtGui.QHBoxLayout()
        self.ch_cam = QtGui.QCheckBox('Camera')
        self.ch_bundle = QtGui.QCheckBox('Bundle')
        self.ch_flipbook = QtGui.QCheckBox('Flipbook')
        self.ch_hide = QtGui.QCheckBox('Hide Other')
        self.ch_cam.setCheckState(Qt.Checked)
        self.ch_bundle.setCheckState(Qt.Checked)
        self.ch_flipbook.setCheckState(Qt.Checked)
        self.ch_hide.setCheckState(Qt.Unchecked)
        ...
        self.btn_addShot = QtGui.QPushButton('Add')
        self.btn_upd = QtGui.QPushButton('Update')
        self.btn_up = QtGui.QPushButton('<   Move UP')
        self.btn_down = QtGui.QPushButton('Move DOWN   >')
        self.btn_delShot = QtGui.QPushButton('Delete')
        self.btn_upd_list = QtGui.QPushButton('Update List')
    ...
    def saveFile(self):
        ...
        file = open(pth, 'w+')
        it = QtGui.QTreeWidgetItemIterator(self.shots)
        while it.value():
            data=[]
            for c in range(0,6):
                val=it.value().text(c)
    ...
    def loadFile(self):
        ...
        if os.path.exists(pth):
            ...
            for i in data:
                ...
                if line:
                    data=i.split(r'***')
                    item = QtGui.QTreeWidgetItem(self.shots,data)
            file.close()
            self.resz()
    ...
    def addShot(self):
        data=self.assemblehotLine().split(r'***')
        item=QtGui.QTreeWidgetItem(self.shots,data)
        self.saveFile()
        self.updateList()

    def updateShot(self):
        item=self.shots.selectedItems()[0]
